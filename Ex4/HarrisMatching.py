...
def harris(im, sigma=1.0, rel_thresh=0.01, k=0.04):
    im = im.astype(np.float64)  # Make sure im is float
...
for i1 in range(npts1):
    patch1 = patches1[:, :, i1]
    p1_numerator = patch1 - np.mean(patch1)
    p1_denominator = np.sqrt(np.sum(p1_numerator ** 2))
    for i2 in range(npts2):
        patch2 = patches2[:, :, i2]
        p2_numerator = patch2 - np.mean(patch2)
        p2_denominator = np.sqrt(np.sum(p2_numerator ** 2))
        if p1_denominator > 0 and p2_denominator > 0:
            ncc = np.sum(p1_numerator * p2_numerator) / (p1_denominator * p2_denominator)
            distmat[i1, i2] = ncc
        else:
            distmat[i1, i2] = 0
...
pairs = []
for k in range(npts1):
    # Check if 1 match 2 and 2 match 1 at the same time
    if k == ids2[ids1[k]]:
        pairs.append(np.array([k, ids1[k], ss1[k]]))
pairs = np.array(pairs)
id_ncc = np.argsort(pairs[:, 2])[::-1]
##-your-code-ends-here-##
