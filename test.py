...
while True:
    ...
    resized_img = imresize(img, (img_height, img_width), preserve_range=True)
    image = np.array([resized_img])
    prediction = model.predict(image)
    print("Predicted results: ", prediction)
    for info in prediction[0]:
        print("Info ", info)
        class_id = int(info[0])
        confidence = info[1]
        if confidence > confidence_threshold:
            x_min, y_min, x_max, y_max = info[2], info[3], info[4], info[5]
            # Change coordinates to original image coordinates.
            left_corner = int(x_min * original_width / img_width)
            top_corner = int(y_min * original_height / img_height)
            right_corner = int(x_max * original_width / img_width)
            bottom_corner = int(y_max * original_height / img_height)
            cv2.rectangle(img, (left_corner, top_corner), (right_corner, bottom_corner), GREEN, rect_thickness)
            label = f"{classes[class_id]}: {confidence}"
            cv2.putText(img, label, (left_corner, top_corner), cv2.FONT_HERSHEY_SIMPLEX, 0.5, GREEN, 2)
    ##-your-code-ends-here-##
